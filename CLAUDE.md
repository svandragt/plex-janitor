...
Requires a `config.ini` (gitignored, not present in repo) with a `[Plex]` section: `username`, `password`, `servername`. Read via `configparser` in `connect()`.
...
2. `connect()` reads Plex credentials from `config.ini` and connects via `plexapi.myplex.MyPlexAccount`.
...
4. `clean_films()`: iterates the "Deletable Films" collection in the "Films" library section and deletes/lists every item unconditionally (no last-one-kept logic).
5. `try_delete()` wraps `item.delete()` and swallows `plexapi.exceptions.BadRequest` and generic exceptions, printing rather than raising.
